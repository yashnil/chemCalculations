#!/usr/bin/env python3
"""
Comprehensive Diagnostics for v10 FastChem ML Emulator

Generates all essential plots and metrics:
1. Global metrics (MAE, R², MSE)
2. Parity plots for top-10 species
3. Residual analysis
4. Per-species error analysis
5. Error distributions
6. Input feature analysis
...
# CONFIGURATION
# =============================================================================
CSV_PATH = '/Users/yashnilmohanty/Desktop/chemCalculations/NEW_VERS/all_gas_v10_no_stripe_clean.csv'
BEST_MODULE = 'runs_mlp_NEW_VERS/best_model.py'
OUT_DIR = 'runs_mlp_NEW_VERS/diagnostics'
...
JITTER_DEX = 0.03  # Jitter for visualization (0.03 dex ≈ 7%)
...
def main():
    ...
    # ─────────────────────────────────────────────────────────────────
    log.info("Loading best_model.py from %s", BEST_MODULE)
    ...
    input_cols = best_mod.INPUT_COLS
    ...
    # Load checkpoint to get split indices
    checkpoint = torch.load(best_mod.CHECKPOINT_PATH, map_location=device)
    config = checkpoint.get("config", {})
    splits = config.get("splits", {})
    
    test_idx = np.array(splits.get("test_idx", []))
    if len(test_idx) == 0:
        log.warning("No test indices in checkpoint; using all data")
    ...
    # 2. Make predictions
    # ─────────────────────────────────────────────────────────────────
    log.info("Normalizing inputs and making predictions...")
    X_test = best_mod.normalize_inputs(df_test)
    
    with torch.no_grad():
        y_scaled_pred = model(X_test).cpu().numpy()
    
    # Denormalize to linear space
    y_pred = best_mod.denormalize_targets(y_scaled_pred)
    
    # Ground truth
    y_true = df_test[target_cols].values.astype(np.float64)
    
    # Clip negatives (shouldn't happen, but safety)
    ...
    # Save metrics
    with open(os.path.join(OUT_DIR, "global_metrics.txt"), "w") as f:
    ...
    # ─────────────────────────────────────────────────────────────────
    # 5. Parity plots for top-10 most abundant species
    # ─────────────────────────────────────────────────────────────────
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, 'parity_top10.png'), dpi=DPI)
    plt.close()
    log.info("✓ Saved: parity_top10.png")
    
    # ─────────────────────────────────────────────────────────────────
    # 6. Overall parity plot (all species pooled)
    # ─────────────────────────────────────────────────────────────────
    ...
    # Subsample if too many points (for plotting speed)
    if len(x_plot) > 50000:
    ...
    if HAS_SCIPY and len(x_plot) > 10:
        ...
        try:
            ...
            ax.hexbin(x_plot, y_plot, xscale='log', yscale='log',
                     gridsize=100, cmap='viridis', mincnt=1)
    else:
        ax.hexbin(x_plot, y_plot, xscale='log', yscale='log',
                 gridsize=100, cmap='viridis', mincnt=1)
    
    # 1:1 line and ±10% band
    lims = [CLIP, 1.0]
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, 'parity_overall.png'), dpi=DPI)
    plt.close()
    log.info("✓ Saved: parity_overall.png")
    
    # ─────────────────────────────────────────────────────────────────
    # 7. MAE per species (bar plot)
    # ─────────────────────────────────────────────────────────────────
    ...
    colors = ['red' if mae > mae_linear else 'steelblue' 
    ...
    ax.axvline(mae_linear, color='black', linestyle='--', linewidth=1, 
               label=f'Global MAE = {mae_linear:.3e}')
    ax.legend()
    ax.grid(True, alpha=0.3, axis='x')
    
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, 'MAE_per_species.png'), dpi=DPI)
    plt.close()
    log.info("✓ Saved: MAE_per_species.png")
    
    # ─────────────────────────────────────────────────────────────────
    # 8. Residual vs observed (scatter)
    # ─────────────────────────────────────────────────────────────────
    ...
    ax.axhline(mae_linear, color='red', linestyle='--', linewidth=1, alpha=0.6, label=f'+MAE')
    ax.axhline(-mae_linear, color='red', linestyle='--', linewidth=1, alpha=0.6, label=f'-MAE')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, 'residual_vs_observed.png'), dpi=DPI)
    plt.close()
    ...
    # ─────────────────────────────────────────────────────────────────
    log.info("Generating error distribution...")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, 'error_distribution.png'), dpi=DPI)
    plt.close()
    log.info("✓ Saved: error_distribution.png")
    
    # ─────────────────────────────────────────────────────────────────
    # 10. Worst predictions analysis
    # ─────────────────────────────────────────────────────────────────
    log.info("Analyzing worst predictions...")
    
    # Compute error per sample (across all species)
    sample_mae = np.mean(np.abs(y_pred - y_true), axis=1)
    worst_indices = sample_mae.argsort()[-100:][::-1]  # Top 100 worst
    
    worst_samples = df_test.iloc[worst_indices].copy()
    worst_samples['sample_MAE'] = sample_mae[worst_indices]
    worst_samples.to_csv(os.path.join(OUT_DIR, 'worst100_samples.csv'), index=False)
    ...
    # ─────────────────────────────────────────────────────────────────
    # 11. Individual species histograms (top-5)
    # ─────────────────────────────────────────────────────────────────
    ...
    for sp in top5_species:
        ...
        if len(obs_vals_nonzero) > 0:
            ax.hist(obs_vals_nonzero, bins=np.logspace(np.log10(CLIP), 0, 80),
        ...
        plt.savefig(os.path.join(OUT_DIR, f'hist_obs_{safe_name}.png'), dpi=DPI)
        plt.close()
    ...
    with open(os.path.join(OUT_DIR, 'diagnostic_summary.txt'), 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("v10 DIAGNOSTIC SUMMARY\n")
        f.write("=" * 70 + "\n\n")
        ...
        for _, row in df_species.nlargest(5, 'mean_abundance').iterrows():
            f.write(f"{row['species']:15s}: mean={row['mean_abundance']:.3e}, "
                   f"MAE={row['MAE']:.3e}, R²={row['R2']:.4f}\n")
        
        f.write("\nTOP-5 SPECIES BY ERROR\n")
        f.write("-" * 70 + "\n")
        for _, row in df_species.nlargest(5, 'MAE').iterrows():
            f.write(f"{row['species']:15s}: MAE={row['MAE']:.3e}, "
        ...
        f.write("• MAE_per_species.png - Bar chart of errors by species\n")
        f.write("• residual_vs_observed.png - Residual analysis\n")
        f.write("• error_distribution.png - Histogram of errors\n")
    ...
    log.info("✓ Saved: diagnostic_summary.txt")
    ...
    log.info("")
    log.info("Key plots:")
    log.info("  • parity_top10.png          - Main diagnostic (2×5 grid)")
    log.info("  • parity_overall.png        - All species pooled")
    log.info("  • MAE_per_species.png       - Error breakdown")
    log.info("  • residual_vs_observed.png  - Residual analysis")
    log.info("  • error_distribution.png    - Error histograms")
    log.info("")
    log.info("Data files:")
    log.info("  • per_species_errors.csv    - Complete error table")
    log.info("  • worst100_samples.csv      - Problematic samples")
    log.info("  • diagnostic_summary.txt    - Text summary")
    log.info("=" * 70)
