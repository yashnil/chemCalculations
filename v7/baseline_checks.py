...
for col in ELEMENT_COLS:
    X[col] = np.log10(X[col]) + 9.0   # bring elements into ~[0,9]
Y = df[species_cols].values.astype("float32")

# ──────────────────────────────────────────────────────────
# 1-D  Train/Val/Test split 60/15/25
# ──────────────────────────────────────────────────────────
all_idx = np.arange(len(df))
# carve off 25% for test
train_val_idx, test_idx = train_test_split(
    all_idx, test_size=0.25, random_state=42, shuffle=True
)
# from the remaining 75%, carve off 20% → 15% total val
train_idx, val_idx = train_test_split(
    train_val_idx, test_size=0.20, random_state=42, shuffle=True
)
...
# persist scaler + *actual index arrays*
joblib.dump(scaler, os.path.join(ARTE_DIR, "input_scaler.pkl"))
np.savez(
    os.path.join(ARTE_DIR, "splits.npz"),
    train_idx=train_idx,
    val_idx=val_idx,
    test_idx=test_idx
)
print("Saved scaler + splits.npz → artefacts/ (with index arrays)")
...
def fastchem_single_point(row):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        inp.temperature, inp.pressure = [row["temperature"]], [row["pressure"]]
...
print(f"FastChem benchmark → {fastchem_ms:.2f} ms/pt")
