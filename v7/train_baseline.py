...
_       = model.predict(X_test[idx,:16], batch_size=16)  # warm‐up
t0 = time.time()
_  = model.predict(X_test[idx],     batch_size=256)
nn_ms = (time.time()-t0)/N_BENCH*1e3
print(f"Inference latency : {nn_ms:.3f} ms/pt  (×{6.12/nn_ms:.1f} vs FastChem)")
