...
GRAD_CLIP_NORM: Optional[float] = 5.0  # Gradient clipping (baseline optimal)
...
HIDDEN: int = 512
DEPTH: int = 3
ACTIVATION: str = "gelu"  # relu | gelu | tanh | sigmoid | leaky_relu
DROPOUT: float = 0.08  # Slight increase for better regularization
...
def compute_target_weights(y_linear: np.ndarray, present_floor: float = 1e-8) -> np.ndarray:
    """Compute per-species weights based on presence frequency in the training set.
    Rare species get higher weights to improve tail accuracy."""
...
class WeightedHuber(nn.Module):
    """Huber loss with per-species weights. More robust than MSE for outliers."""
...
class EMA:
    """Exponential Moving Average of model weights for smoother, better generalization."""
...
def _species_candidates(df: pd.DataFrame, input_cols: Sequence[str]) -> List[str]:
    ...
    # Exclude comp_* columns (elemental composition metadata, not true species targets)
    return [c for c in numeric_cols if (c not in exclude and not _is_abund_col(c) and not c.startswith("comp_"))]
...
class ResBlock(nn.Module):
    """Residual block with skip connection for smoother optimization."""
...
class MLP(nn.Module):
    """Residual MLP with skip connections for improved gradient flow."""
...
def main() -> None:
    ...
    # Switch to plain MSE loss for better R² optimization (keeping clean targets & ResNet arch)
    opt = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
    sch = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=EPOCHS, eta_min=ETA_MIN)
    loss_fn = nn.MSELoss()
    log.info("Using MSE loss (optimized for R² metrics)")
    log.info("Gradient Clipping: max_norm=%.1f (tighter control for rare species)", GRAD_CLIP_NORM)
    log.info("EMA disabled - using direct training weights for simplicity")
