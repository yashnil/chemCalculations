...
def main():
    ...
    n_groups = 10
    n_points_per_group = 400

    all_gas_list = []
    all_cond_list = []

    # Create one RNG for the entire run for reproducibility
    rng = np.random.default_rng(seed=12345)

    overall_start = time.time()

    # Loop over groups (for organizational purposes)
    for group in range(n_groups):
        group_gas_list = []
        group_cond_list = []
        print(f"\n=== Processing Group {group} with {n_points_per_group} independent evaluations ===")
        group_start = time.time()

        # Loop over independent evaluations in this group
        for pt in range(n_points_per_group):
            # Sample T and P independently
            T_val = rng.uniform(100.0, 3000.0)
            logP_val = rng.uniform(-10.0, 5.0)
            P_val = 10 ** logP_val

            # Generate a unique composition for this evaluation
            comp = generate_single_composition()

            # For this evaluation, T and P are provided as one-element arrays
            T_array = np.array([T_val])
            P_array = np.array([P_val])

            # Create an output directory for this individual run
            out_dir = os.path.join("results", f"group{group}", f"point{pt}")
            df_gas, df_cond, flags = run_pyfastchem_for_composition(
                comp,
                T_array,
                P_array,
                out_dir=out_dir,
                condensates=False  # set to True if using condensates
            )

            # Tag with group and point indices
            df_gas['group_index'] = group
            df_gas['point_index'] = pt
            df_cond['group_index'] = group
            df_cond['point_index'] = pt

            group_gas_list.append(df_gas)
            group_cond_list.append(df_cond)

        # Combine all evaluations in the current group
        df_group_gas = pd.concat(group_gas_list, ignore_index=True)
        df_group_cond = pd.concat(group_cond_list, ignore_index=True)

        # Normalize gas-phase species abundances for each evaluation point
        df_group_gas_normalized = normalize_gas_abundances(df_group_gas)

        # Determine the top-10 gas-phase species based on normalized abundances
        top10 = find_top_10_species(df_group_gas_normalized)
        print(f"    -> Top-10 gas-phase species for group {group}: {top10}")

        # Plot each of the top species versus temperature and pressure
        plot_species_vs_TP(df_group_gas_normalized, top10, group_index=group)

        all_gas_list.append(df_group_gas_normalized)
        all_cond_list.append(df_group_cond)

        group_end = time.time()
        print(f"=== Group {group} finished in {group_end - group_start:.2f} seconds.")

    overall_end = time.time()
    print("\n=== All independent evaluations complete! ===")
    print(f"Total run time: {overall_end - overall_start:.2f} seconds.")

    # Merge and save all results
    df_all_gas = pd.concat(all_gas_list, ignore_index=True)
    df_all_cond = pd.concat(all_cond_list, ignore_index=True)
    ...
    print("Check the specified directories for saved plots and CSV tables.\n")
