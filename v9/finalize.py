...
gamma = float(best.get("gamma", 1.6))
alpha = float(best.get("alpha_entropy", 1e-3))
...
model.add(softplus_head(len(species), gamma=gamma))

model.compile(
    optimizer=keras.optimizers.Adam(best["lr"]),
    loss=composite_loss(lam=0.1, beta=0.0, alpha_entropy=alpha),
    metrics=[_mae_log]
)
