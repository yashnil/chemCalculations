...
# ╭──────────────── 9.  export gross outliers for forensics ─────╮
OUTLIER_THRESH = 1e3    # |pred/obs| > 1e3  or  |obs/pred| > 1e3 (symmetric)
EPS2           = 1e-20  # tiny guard for divisions / logs

def build_outlier_frame(species_subset):
    # indices of the chosen species
    sp_idx = [SPECIES.index(s) for s in species_subset]

    # slice arrays for speed
    yt = Y_true[:, sp_idx] + EPS2
    yp = Y_pred[:, sp_idx] + EPS2

    # symmetric ratio: max(pred/true, true/pred)
    ratio = np.maximum(yp / yt, yt / yp)

    # boolean mask of big deviations
    mask = ratio > OUTLIER_THRESH
    rows, cols = np.where(mask)

    if rows.size == 0:
        return pd.DataFrame(columns=[
            "row_idx","species","true","pred","ratio",
            "log10_true","log10_pred","temperature","log10_pressure",
            "comp_H","comp_O","comp_C","comp_N","comp_S"
        ])
    ...
    data = {
        "row_idx"       : rows,
        "species"       : [species_subset[j] for j in cols],
        "true"          : yt[rows, cols],
        "pred"          : yp[rows, cols],
    }
    df_out = pd.DataFrame(data)
    df_out["ratio"]       = np.maximum(df_out["pred"]/df_out["true"], df_out["true"]/df_out["pred"])
    df_out["log10_true"]  = np.log10(df_out["true"])
    df_out["log10_pred"]  = np.log10(df_out["pred"])

    # join inputs
    meta_cols = ["temperature","pressure","comp_H","comp_O","comp_C","comp_N","comp_S"]
    meta_df   = df.loc[df_out["row_idx"], meta_cols].reset_index(drop=True).copy()
    meta_df["log10_pressure"] = np.log10(meta_df["pressure"].values + EPS2)
    df_out = pd.concat([df_out.reset_index(drop=True), meta_df.drop(columns=["pressure"])], axis=1)

    # handy extras for quick slicing later
    df_out["abs_log_error"] = (df_out["log10_pred"] - df_out["log10_true"]).abs()
    return df_out.sort_values("ratio", ascending=False).reset_index(drop=True)

# all species
df_out_all = build_outlier_frame(SPECIES)
df_out_all.to_csv(os.path.join(OUT_DIR, "gross_outliers_all.csv"), index=False)

# restricted to the same "top-10 by max abundance" used in panels
top10_species = tbl.sort_values("max_abun", ascending=False)["species"].head(10).tolist()
df_out_top10 = build_outlier_frame(top10_species)
df_out_top10.to_csv(os.path.join(OUT_DIR, "gross_outliers_top10.csv"), index=False)

print(f"Outlier CSVs written → {OUT_DIR}/gross_outliers_all.csv  and  gross_outliers_top10.csv")
# ╰──────────────────────────────────────────────────────────────╯
