...
#!/usr/bin/env python3
# step5_diagnostics.py  –  deep‑dive error analysis for the FastChem surrogate
# ---------------------------------------------------------------------------
...
# ──────────────────────────────────────────────────────────────────────────
top10 = (Y_true.max(axis=0)).argsort()[-10:][::-1]     # indices of 10 highest‑peak species
fig, axs = plt.subplots(2,5, figsize=(15,6))
axs = axs.ravel()
for ax, idx in zip(axs, top10):
    ax.scatter(Y_true[:,idx], Y_pred[:,idx], s=4, alpha=0.4)
    lim = [0, max(Y_true[:,idx].max(), Y_pred[:,idx].max())]
    ax.plot(lim, lim, 'r--', lw=1)
    ax.set_title(SPECIES[idx])
...
plt.title(f"Residuals for worst‑MAE species : {sp}")
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, f"residual_TP_{sp}.png"), dpi=180)
plt.close()

# ──────────────────────────────────────────────────────────────────────────
# 6)  MAE vs. species (bar chart – optional)
# ──────────────────────────────────────────────────────────────────────────
plt.figure(figsize=(12,4))
rank = np.argsort(mae_sp)
plt.bar(np.arange(len(SPECIES)), mae_sp[rank])
plt.xticks(np.arange(len(SPECIES)), np.array(SPECIES)[rank], rotation=90, fontsize=6)
plt.ylabel("MAE")
