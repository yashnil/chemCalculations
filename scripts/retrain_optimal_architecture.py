#!/usr/bin/env python3
"""
retrain_optimal_architecture.py
================================

Retrain models for dataset sizes 160, 320, 480, 640, 800 using the optimal architecture
from x160_static_32_consistent (the best 160K model).
...
def train_model(size: int, config_path: Path):
    ...
    run_dir = BASE_DIR / "results" / "runs" / f"runs_autoencoder_x{size}_optimal_retrained"
...
def main():
    print("="*80)
    print("RETRAINING MODELS WITH OPTIMAL ARCHITECTURE")
    print("="*80)
    ...
    if missing:
        ...
        print("   These datasets need to be generated first.")
        print("   Check scripts/ or data/ directories for dataset generation scripts.")
        response = input("\nContinue with available datasets only? (y/n): ")
        if response.lower() != 'y':
            print("Aborted.")
            return
